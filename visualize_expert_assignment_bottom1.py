"""Visualize bottom-1 expert (token -> least probable expert) as a grid PNG.

Same layout as visualize_expert_assignment.py, but each token is colored by the
expert that received the *lowest* softmax probability (bottom-1) from the router,
instead of the top-1 selected expert.

  - rows   = images
  - col 0  = original image only
  - col 1.. = original image + layer 0, 1, ... bottom-1 expert overlay (semi-transparent)

Uses forward hooks on each Top2Gating module and reads last_raw_gates to compute
argmin (bottom-1 expert id per token).
...
class ExpertAssignmentCapture:
    """Capture per-block bottom-1 expert id (token -> expert with lowest softmax prob)."""
    ...
    def _make_gate_hook(self, block_id: int):
        def hook_fn(module, input, output):
            ...
            # Bottom-1: expert with smallest softmax probability per token
            idx = raw_gates.argmin(dim=-1)  # (B, N)
            idx0 = idx[0].detach().cpu().to(torch.int64).numpy()
            idx_hw = idx0.reshape(H, W)

            if block_id == 0:
                bottom1_expert = int(idx_hw[0, 0])
                print(
                    f"[debug] block 0, token (r=0, c=0) bottom-1 expert id = {bottom1_expert}"
                )

            self.assignments[block_id] = (H, W, idx_hw)

            if self.num_experts is None:
                self.num_experts = int(E)
...
def main():
    ...
    print(f"Running {num_images} image(s) starting at idx={start_idx} (bottom-1 expert) ...")
    with torch.no_grad():
        seen = 0
    ...
    out_path = os.path.join(args.output_dir, "expert_assignment_bottom1_grid.png")
    save_grid_png(
        grid,
