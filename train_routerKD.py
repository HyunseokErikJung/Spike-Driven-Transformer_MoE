...
# ------------------------------
# Router self-KD (last block teacher)
# ------------------------------
parser.add_argument(
    "--router-kd",
    action="store_true",
...
parser.add_argument(
    ...
    type=float,
    default=1.0,
    help="Base weight for router KD loss (multiplied by warmup schedule if enabled).",
)
parser.add_argument(
    "--router-kd-temp",
    type=float,
...
parser.add_argument(
    "--router-kd-warmup-epochs",
    type=int,
    default=0,
    help="Warmup epochs for router KD (0 disables warmup).",
)
...
def _apply_temp_to_probs(probs: torch.Tensor, temp: float, eps: float = 1e-8) -> torch.Tensor:
    # probs: (B, N, E), already softmax probabilities from router
    # use log(probs) as logits proxy and re-softmax with temperature
...
def router_self_kd_kl(
    ...
    """
    KL( teacher^T || student^T ) averaged over (B,N) and students, scaled by T^2.
    All inputs are probability tensors of shape (B, N, E).
    """
    ...
    # teacher is treated as target distribution (stop-grad)
    ...
    for p_s_raw in student_probs_list:
        ...
        kl = (p_t * (log_p_t - log_p_s)).sum(dim=-1)  # (B, N)
...
def train_one_epoch(
    ...
    for batch_idx, (input, target) in enumerate(loader):
        ...
        with amp_autocast():
            ...
            # Router self-KD: blocks[0..L-2] match blocks[L-1] routing distribution
            kd_loss = None
            kd_w = 0.0
                ...
                if kd_w > 0:
                    ...
                    try:
                        ...
                        if blocks is not None and len(blocks) >= 2:
                            teacher = blocks[-1].mlp.gate.last_raw_gates  # (B,N,E)
                            students = [
                                blocks[i].mlp.gate.last_raw_gates for i in range(len(blocks) - 1)
                            ]
